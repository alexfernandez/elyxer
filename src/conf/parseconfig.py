...
class ConfigTranslator(object):
  ...
  def write(self, objects):
    ...
    for name, current in classes:
      self.writeclass(self, name, current)

  def writeclass(self, name, current):
    "Write an object class"
    self.writer.writeline('class ' + name + '(object)')
    self.writer.writeline('  "Autogenerated from conf.config file"')
    self.writer.writeline('')
    for attrname, contents in current:
      self.writeattr(attrname, contents)
  ...
  def writelist(self, values):
    ...
    for value in values:
      self.writer.writestring('u"' + value + '", ')
    self.writer.writestring(']')
  ...
  def writedict(self, values):
    ...
    for key, value in values.iteritems():
      self.writer.writestring('u"' + key + '":u"' + value + '", ')
    self.writer.writestring('}')
